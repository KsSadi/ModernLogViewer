      ...
      (set, get) => ({
        ...
        getFilteredEntries: (fileId) => {
          ...
          return file.entries.filter(entry => {
            // Level filter (support both single and multiple levels)
            if (filters.level && entry.level !== filters.level) return false
            
            // Multiple levels filter
            const activeLevels = filters.levels ? Object.keys(filters.levels).filter(key => filters.levels[key]) : []
            if (activeLevels.length > 0 && !activeLevels.includes(entry.level)) return false
